...
from src.models import SubspaceNet, TransSubspaceNet
...
def evaluate_dnn_model(model, dataset: list, criterion: nn.Module,
    ...
    """
    Evaluate the DNN model on a given datasets.

    Args:
        model (nn.Module): The trained model to evaluate.
        dataset (list): The evaluation datasets.
    ...
    with torch.no_grad():
        for data in dataset:
            if model_type.startswith("Trans2DSubNet"):
                X, DOA_t, DOA_p = data
                test_length += DOA_t.shape[0]
                # Cast observations and DoA to Variables
                X = X.to(device)
                DOA_t_pred, DOA_p_pred = model(X)
                DOA_t = DOA_t.to(device)
                DOA_p = DOA_p.to(device)
                ...
                elif model_type.startswith("TransSubspaceNet"):
                    # Default - SubSpaceNet
                    DOA_predictions = model_output[0]
                else:
            ...
            if model_type.startswith("DeepCNN") and isinstance(criterion, RMSPELoss):
                ...
                loss_t = criterion(DOA_t_pred, DOA_t)
                loss_p = criterion(DOA_p_pred, DOA_p)
                eval_loss = (loss_t + loss_p) / 2
            else:
                eval_loss = criterion(DOA_predictions, DOA)
            # add the batch evaluation loss to epoch loss              
            overall_loss += eval_loss.item()
            # overall_loss_t += loss_t.item()
            # overall_loss_p += loss_p.item()
        overall_loss = overall_loss / test_length
        # overall_loss_t /= test_length
        # overall_loss_p /= test_length
    # Plot spectrum for SubspaceNet model
    if plot_spec:
        if model_type.startswith("SubspaceNet") or model_type.startswith("TransSubspaceNet"):
    ...
    return overall_loss  #, overall_loss_t, overall_loss_p
...
def evaluate_augmented_model(model: SubspaceNet, dataset, system_model, criterion=RMSPE,
    ...
    """
    Evaluate an augmented model that combines a SubspaceNet model with another subspace method on a given datasets.
...
def evaluate_model_based(dataset: list, system_model, criterion: RMSPE,
    ...
    """
    Evaluate different model-based algorithms on a given datasets.
...
def evaluate_classifier(model, dataset: list, criterion):
    """
    Evaluate the Classifier model on a given datasets.
    """
    ...
    with torch.no_grad():
        for data in dataset:
            X, true_num = data
            X = X.to(device)
            true_num = true_num.to(device)
            eval_length += true_num.shape[0]

            logits, pred_num = model(X)

            eval_loss = criterion(logits, true_num)
            overall_loss += eval_loss.item()

            is_equal = pred_num.eq(true_num.squeeze())
            overall_acc += is_equal.sum().item()

        overall_loss /= len(dataset)
        overall_acc /= eval_length
...
def evaluate(
    ...
    Parameters:
        ...
        model_test_dataset (list): Test datasets for the model.
        generic_test_dataset (list): Test datasets for generic subspace methods.
    ...
    if not isinstance(augmented_methods, list) and model_type.startswith(("SubspaceNet", "TransSubspaceNet")):
        augmented_methods = ["music", "esprit"]

    # Set default model-based subspace methods
    if not isinstance(subspace_methods, list):
        subspace_methods = ["esprit", "music", "r-music", "sps-r-music",
                            "sps-esprit", "sps-music"]  # "mvdr", "bb-music",
    ...
    model_test_loss = evaluate_dnn_model(model=model, dataset=model_test_dataset,
                                         criterion=criterion, plot_spec=plot_spec, figures=figures,
