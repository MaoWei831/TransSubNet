...
def plot_music_spectrum(system_model, figures: dict, spectrum: np.ndarray, algorithm: str):
    ...
    plt.savefig("./data/spectrums/{}_music_spectrum.pdf".format(algorithm), bbox_inches='tight')
...
def plot_mvdr_spectrum(system_model, figures: dict, spectrum: np.ndarray,
    ...
    plt.savefig("./data/spectrums/{}_mvdr_spectrum.pdf".format(algorithm), bbox_inches='tight')
...
def plot_in_mystyle(x, y):
    ...
    ax.plot(x, y,
            ...
            markersize=10,
            label="TransSubNet")
    ax.set_xticks(np.linspace(-5, -1, 1), font={'family': 'Times New Roman', 'size': 12})
    ax.set_yticks(np.linspace(-40, -25, 5), font={'family': 'Times New Roman', 'size': 12})
    ax.set_xlabel("SNR[dB]", font={'family': 'Times New Roman', 'size': 16})
    ax.set_ylabel("MSPR[dB]", font={'family': 'Times New Roman', 'size': 16})
    # ax.set_title()
    ax.legend(loc='best', font={'family': 'Times New Roman', 'size': 12})
